...
# This module define a function to plot multiple plots of embedding points of single cell data.
import cooltools
import numpy as np
import pandas as pd
...
def plot_altitude(vectors, titles,n_row=10, n_col=2):
    # plot the Altitude graph for the vectors
    plt.figure(figsize=(5 * n_col, 2 * n_row))
    plt.subplots_adjust(bottom=0, left=.01, right=.99, top=.90, hspace=.35)
    for i in range(n_row * n_col):
        plt.subplot(n_row, n_col, i + 1)
        plt.fill_between(np.arange(len(vectors[i])),0,vectors[i])
        plt.title(titles[i], size=12)
        plt.xticks(())
        plt.yticks(())


def decomposition_mainvector(mat,pc = 0,drop_kinetochore = False,save_fig = False,name = 'name'):
    warnings.filterwarnings("ignore") 
    if drop_kinetochore == True :
        zeroline = np.where(sum(mat) == 0)
        mat = np.delete(mat,zeroline,axis = 0)
        mat = np.delete(mat,zeroline,axis = 1)
        print('kinetochore droped')
    if save_fig == True :
        corrmap(mat,name)
        pcamap(mat,name)
    corr = np.corrcoef(oe(mat))  
