...
def calculate_embedding(cool_path,
                     ...
                     **kwargs) -> pd.Series:
    """
    calculate_embedding transform 2D matrix to 1D embedding, achieve dimensionality reduction.
    ...
    if operation in operation_dict.keys():
        ...
        print(f"Not a valid embedding method! Now only support {list(operation_dict.keys())}")    
...
def loop_APA(cool_path, loop_anchors, balance, field, width=20, pos="lower-right"):
    ...
    for loop in loop_anchors:
        ...
        if len < min_len:
            ...
            r1 = chromosome + ":" + \
                str(start - width*binsize) + "-" + \
                str(start + (1+width)*binsize)
            r2 = chromosome + ":" + \
                str(end - width*binsize) + "-" + str(end + (1+width)*binsize)
        ...
        if pos == None:
            ...
            backgroud = (mat[-n:, -n:]+mat[:n, -n:] +
                         mat[:n, :n]+mat[-n:, :n]).mean()
        else:
...
def strata_strength(cool_path, chrom, balance, field, n_strata=20):
    all_strata = []
    mat = cooler.Cooler(cool_path).matrix(
        balance=balance, field=field).fetch(chrom)[:]
...
def conservation_region(cool_path, regions, balance, field, intra_chrom=True):
    ...
    for region in regions:
        intra_region = clr.matrix(
            balance=balance, field=field, as_pixels=True).fetch(region)[field].sum()
        if intra_chrom:
            chrom = region.split(":")[0]
            background_region = clr.matrix(
                balance=balance, field=field, as_pixels=True).fetch(region, chrom)[field].sum()
        else:
...
def strength_region(cool_path, regions, balance, field, intra_chrom=True, percentage=False):
    ...
    for region in regions:
        if intra_chrom:
            result.append(clr.matrix(balance=balance,
                          field=field).fetch(region).mean())
        else:
            chrom = region.split(":")[0]
            intra = clr.matrix(balance=balance, field=field, as_pixels=True).fetch(
                region, chrom)[field].sum()
...
def node_degree(cool_path, chrom, balance, field):
    clr = cooler.Cooler(cool_path)
    pixels = clr.matrix(balance=balance, as_pixels=True,
                        field=field).fetch(chrom)
...
def _node_conservation(cool, bin, chrom="chr1"):
    ...
    bins = pd.merge(bin1, bin2, on=bin1.index.names,
                    how="outer").fillna(0).sum(axis=1)
    ...
    for i in index_set:
        conserve["conservation"][i] = bins[i].get(
            1, 1) / (bins[i].get(-1, 1) + bins[i].get(1, 1))
...
def all_strata(cell_list, chrom="chr1", n_strata=20, field="count", concat=False):
    ...
    for cell in cell_list:
        ...
        mat = cooler.Cooler(cell).matrix(
            balance=False, field=field).fetch(chrom)[:]
