...
def cal_minhash(cell,chrom = "chr1"):
...
def _cal_minhash(   cell,
                    chroms = ["chr1"],
                    method = "intra",
                    n_strata = None,
                    keep_short = True,
                    keep_all_strata = False):
    pixels = pd.DataFrame()
    clr = cooler.Cooler(cell)
    if method == 'intra':
        for i in chroms:
...
def minhash_similarity( cell_list,
                        chroms = ["chr1"],
                        method = "intra",
                        n_strata = None,
                        keep_short = True):
    cal_minhash1 = partial(_cal_minhash,chroms=chroms,method=method,n_strata=n_strata,keep_short=keep_short)
...
def pairwise_distances(all_strata, 
                       similarity_method,
                       print_time=False, 
                       sigma=.5, 
                       window_size=10,
