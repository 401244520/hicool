...
try:
    ...
    print("Please install pytorch if you want to load data to_tensor, now using multiprocessing and numpy.")
    to_tensor = False
...
def load_cool_adj(cool_path: str,
    ...
    pixels = clr.matrix(as_pixels=True, balance=balance,
                        field=field).fetch(chrom, chrom)
...
if to_tensor:
    class CoolData(data.Dataset):
        def __init__(self,
            ...
            self.cell_list = [
                scool_path + "::" + c for c in cooler.fileops.list_scool_cells(scool_path)]
            self.chrom = chrom
        ...
        def __getitem__(self, index):
            ...
            data_tensor = load_cool_adj(
                cell, self.chrom, self.sparse, self.field)  # ,self.transform)
            if self.label_list:
    ...
    def load_scool_adj(scool_path,
        ...
        dataloader = data.DataLoader(
            scool_data, batch_size=len(scool_data), num_workers=nproc)
        if return_loader:
            return dataloader
    ...
    def load_scool_adj(scool_path,
        ...
        cell_list = [scool_path + "::" +
                     c for c in cooler.fileops.list_scool_cells(scool_path)]
        load_adj = partial(load_cool_adj, chrom=chrom, sparse=sparse,
                           field=field, to_tensor=False)  # ,transform=transform)
...
def load_cool_region(cool_path,
    ...
    if as_graph:
        ...
        result = clr.matrix(as_pixels=as_pixels,
                            balance=False, field=field).fetch(region)
    return result
...
def load_scool_region(scool_path: str,
    ...
    cell_list = [scool_path + "::" +
                 c for c in cooler.fileops.list_scool_cells(scool_path)]
    load_region = partial(load_cool_region, region=region,
                          as_pixels=as_pixels, field=field)
    with Pool(processes=nproc) as pool:
