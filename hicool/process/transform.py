...
def txt2cool(file_path,
             chroms,
             positions,
             resolution,
             chrom_list,
             chromsizes):
    file = AutoLoad(file_path,resolution=resolution)
...
def txt2scool(file_path,
    ...
    Default use multiprocessing.Pool, or you can directly use txt2cool with a loop.
    Attetion: text files default split by '\t' and need chromsome and genome position (not bed format, one location is enough) for each pair at least
    ...
    file = AutoLoad(file_path[0],resolution=resolution)
