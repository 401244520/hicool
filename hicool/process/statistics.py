...
class PreprocessCooler():
    ...
    def cal_intra_slow(self):
        ...
        px = self.px
        intra = px[px.bin1_id.apply(lambda x: self.bins.chrom[x]) == px.bin2_id.apply(
            lambda x: self.bins.chrom[x])]["count"].sum()
        return intra

    def cal_intra(self):
        # calculate hic intra-chromosome contacts, which means conatcts within each chromsomes. inter = total - intra
        intra = [self.clr.matrix(balance=False, as_pixels=True).fetch(i)[
            "count"].sum() for i in self.clr.chromnames]
        return np.sum(intra)
    ...
    def cal_short_range(self):
        ...
        px = self.px
        short_range = px[px.bin2_id - px.bin1_id <
                         2000000/self.resolution]['count'].sum()
        return short_range

    def cal_mitotic(self):
        # calculate hic contacts less than 12 MB and more than 2MB genomic distance
        px = self.px
        ind = (px.bin2_id - px.bin1_id >= 2000000 /
               self.resolution) & (px.bin2_id - px.bin1_id < 12000000/self.resolution)
    ...
    def cal_long_range(self):
        ...
        px = self.px
        short_range = px[px.bin2_id - px.bin1_id >=
                         12000000/self.resolution]['count'].sum()
        return short_range
...
def preprocess(cool_url):
    ...
    stat = [cool_url, amount, nonzero_bins,
            intra, short_range, mitotic, long_range]
    return stat
...
def scool_meta(file_list,
    ...
    label["Sample Name"] = label["RawPath"].apply(
    ...
    print(
        f"Create raw meta data transform from {runtable_path} to match all txt files... \nStore raw meta to {meta_path}")
    return meta_path
...
def scool_meta_dipc2021(txt_path="/store/zlwang/Workspace/data/Dip-C/pairs/GSM*", runtable_path="/store/zlwang/Workspace/data/Dip-C/GSE146397_metadata.cells_contacts_100k.txt", prefix='DipC2021', save_dir="."):
    ...
    label["SampleName"] = label.path.apply(
        lambda x: os.path.basename(x).split(".")[0])
    label['label'] = label["SampleName"].apply(
        lambda x: RunTable_dict.get(x[11:]))
    meta_path = save_dir + "/" + prefix + "_raw_meta.csv"
...
def scool_meta_nagano(scool_path="/store/zlwang/Workspace/data/Nagano2017/nagano_10kb_cell_types.scool", table_path="/store/zlwang/Workspace/data/Nagano2017/GSE94489_README.txt", prefix='Nagano2017', save_dir="."):
    ...
    ind["Five_prime_barcode"] = [
        i.split("/")[-1].split("_")[2] for i in cell_list]
    ind["Three_prime_barcode"] = [
        i.split("/")[-1].split("_")[3] for i in cell_list]
    ind["Nature_Reference_ID"] = [i.split(
        "/")[-1].split("_")[0] + "_" + i.split("/")[-1].split("_")[1] for i in cell_list]
    meta = pd.merge(ind, rdm, how="left", on=[
                    "Five_prime_barcode", "Three_prime_barcode", "Nature_Reference_ID"])
...
def check_meta(scool, meta_path, rawpath_col=0, sample_col=1, label_col=2):
    ...
    if len(cells) != len(meta):
        raise Exception(
            f"metadata({len(meta)}) is not equal to cells({len(cells)})!")
    cell_set = {c.split("/")[-1] for c in cells}
    meta_set = {m.split("/")[-1].split(".")[0]
                for m in meta.iloc[:, rawpath_col].values}
    meta_set_full = {m.split("/")[-1]
                     for m in meta.iloc[:, rawpath_col].values}
    if not (cell_set.issubset(meta_set) or cell_set.issubset(meta_set_full)):
        inter = meta_set.intersection(cell_set)
        raise Exception(
            f"metadata({len(meta_set)}) not fully include scool cells({len(cell_set)}), which have {len(inter)} intersect.")
    # Check name duplicate
    name_set = set(meta.iloc[:, sample_col])
    if len(name_set) != len(meta):
        raise Exception(
            f"sample name({len(meta)}) is not unique({len(name_set)})!")
    # Check label
...
def scool_filer_cells(scool_path, cell_list, qual_path=None):
    ...
    all_cells = set(cell.split("/")[-1]
                    for cell in cooler.fileops.list_scool_cells(scool_path))
    if all_cells.issuperset(set(cell_list)):
        del_cells = all_cells - set(cell_list)
    else:
        raise Exception(
            "Not all cells provided in cell_list in scool file. Double check your cell list and scool cells.")
...
def quality_control(scool_path,
    ...
    meta_path : str, optional
        Meta information of txt format, suggest provide meta information for the following analysis.
        Which need at least three columns: rawpath, sample name, labels. 
        You can use scool_meta create it from SRA RunTable or manually.
    ...
    with Pool(processes=nproc) as pool:
        result = list(
            tqdm(pool.imap(preprocess, cell_list), total=len(cell_list)))
    stats = pd.DataFrame(result, columns=[
                         "cool_url", "total_contacts", "nonzero_bins", "intra", "short_range", "mitotic", "long_range"])
    if meta_path is not None:
    ...
    stats_info = stats[["total_contacts", "nonzero_bins",
                        "intra(%)", "short(%)", "mitotic(%)", "long(%)"]]
    qc_dict = {"total_contacts": min_cutoff,
               "nonzero_bins": nonzero_cutoff, "intra(%)": intra_cutoff}
    ...
    if save_pass:
        ...
        print("Saving quality control cells results to " +
              qual_scool + " and  " + qual_path)
...
def plot_stats(qual_path):
    ...
    stats_info = stats[["total_contacts", "nonzero_bins",
                        "short(%)", "mitotic(%)", "long(%)", "intra(%)", "label"]]
...
def distance_frequency_cruve(cell_list, chrom="chr1"):
    ...
    for cell in cell_list:
        ...
        if chrom == "all":
            ...
            pixels = clr.matrix(
                as_pixels=True, balance=False).fetch(chrom, chrom)
    ...
    plt.fill_between(dis_sum.columns, dis_sum.mean() -
                     dis_sum.var(), dis_sum.mean()+dis_sum.var(), alpha=0.2)
    plt.xlim(-5, 100)
    plt.legend(["percentage", "percentage variance",
               "total percentage", "total percentage variance"])
