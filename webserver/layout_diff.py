...
def hicool_diff(datasets):
    ...
    return html.Div([
        ...
        html.Div([
            html.Div([
                dcc.Dropdown(id='hicool_cell1',
                )
            ],style={'display': 'inline-block','textAlign': 'center', 'width': '25%'}),
            html.Div([
                dcc.Dropdown(id='hicool_cell2',
                )
            ],style={'display': 'inline-block','textAlign': 'center', 'width': '25%'}),
        ]),
        
        html.Div([
            dcc.Dropdown(id='hicool_features', multi=True)
        ], style={'display': 'inline-block','textAlign': 'center', 'width': '50%'}),
        
        html.Div([
            html.Div([
                dcc.Dropdown(id='hicool_chrom1')
            ], style={'display': 'inline-block','textAlign': 'center', 'height': '22px', 'width': '12%'}),
            html.Div([
                dcc.Input(id='hicool_start1', placeholder='start:0', 
                        type='int', min=0, value=0, style={'height': '30px','textAlign': 'center'}),        
            ], style={'display': 'inline-block', 'width': '10%','margin': '0 1%'}),
            html.Div([
                dcc.Input(id='hicool_end1', placeholder='end:', type='int',
...
def update_graph(cell1,
    ...
    if cell2 is not None:
        ...
        matrix = np.tril(mat1) + np.triu(mat2)
    else:
